...
FAILURE_KIND_MISSING_ASSET = "missing_asset"
FAILURE_KIND_INFERENCE_ERROR = "inference_error"
FAILURE_KIND_UNSUPPORTED_MODALITY = "unsupported_modality"
...
CREATE TABLE IF NOT EXISTS jobs (
    ...
    started_at TEXT,
    finished_at TEXT,
    UNIQUE (clip_id, model_id, modality, language, prompt_id)
);
...
CREATE TABLE IF NOT EXISTS job_verdicts (
    ...
    verdict TEXT NOT NULL CHECK (verdict IN (
        'SARCASTIC', 'NOT_SARCASTIC', 'LLM_ERR'
    )),
    sarcastic INTEGER,
    confidence INTEGER,
    parse_error TEXT,
...
class Database:
    ...
    def upsert_prompt(
        self, conn: sqlite3.Connection, slug: str, filename: str
    ) -> int:
        conn.execute(
            """
            INSERT INTO prompts (slug, filename)
            VALUES (?, ?)
            ON CONFLICT(slug) DO UPDATE SET filename = excluded.filename
            """,
            (slug, filename),
        )
        row = conn.execute("SELECT id FROM prompts WHERE slug = ?", (slug,)).fetchone()
        assert row is not None
        return int(row["id"])

    def list_prompts(self, conn: sqlite3.Connection) -> list[tuple[int, str, str]]:
        rows = conn.execute(
            "SELECT id, slug, filename FROM prompts ORDER BY slug"
        ).fetchall()
        return [(int(r["id"]), str(r["slug"]), str(r["filename"])) for r in rows]
                ...
                for modality, language in available:
                    cur = conn.execute(
                        ...
                        INSERT OR IGNORE INTO jobs (
                            clip_id, model_id, prompt_id, modality, language
                        ) VALUES (?, ?, ?, ?, ?)
                        """,
                        (clip_id, model_id, prompt_id, modality, language),
                    )
        ...
        if model_id is None:
            row = conn.execute(
                ...
                SELECT j.id, j.clip_id, j.model_id, m.name AS model_name,
                       j.prompt_id, p.slug AS prompt_slug,
    ...
    def finish_job(
        ...
        *,
        ...
        conn.execute(
            """
            UPDATE jobs
            SET status = ?, finished_at = ?, last_error = ?, failure_kind = ?
            WHERE id = ?
            """,
            (status, utc_now(), last_error, failure_kind, job_id),
        )
    ...
    def requeue_job(
        ...
        *,
        ...
        conn.execute(
            """
            UPDATE jobs
            SET status = 'pending', started_at = NULL, last_error = ?, failure_kind = NULL
            WHERE id = ?
            """,
            (last_error, job_id),
        )
    ...
    def failure_kind_counts(self, conn: sqlite3.Connection) -> dict[str, int]:
        rows = conn.execute(
            """
            SELECT failure_kind, COUNT(*) AS cnt FROM jobs
            WHERE failure_kind IS NOT NULL
            GROUP BY failure_kind
            """
        ).fetchall()
        return {str(r["failure_kind"]): int(r["cnt"]) for r in rows}
    ...
    def delete_job_verdict(self, conn: sqlite3.Connection, job_id: int) -> None:
        conn.execute("DELETE FROM job_verdicts WHERE job_id = ?", (job_id,))
